!function ($) {
    if (!Array.prototype.map) {
        Array.prototype.map = function (fn, that) {
            var i = 0, ilen = this.length, resultA = [];
            for (; i < ilen; ++i)resultA.push(fn.call(that, this[i], i, this));
            return resultA
        }
    }
    var plugin_name = "rta", defaultButtons = "bold italic underline strike," + "ul ol, link image", allButtons = "bold italic underline strike," + "font-name font-size font-color font-bgcolor," + "font-increase font-decrease," + "superscript subscript," + "align-left align-center align-right align-full," + "indent-right indent-left," + "ul ol," + "link unlink image," + "heading paragraph div hr," + "h1 h2 h3 h4 h5 h6," + "bgcolor," + "remove-formatting html,", rxPlaceholder = /<span class=['"]?rta-placeholder['"]?>.*?<\/span>/i, isPlaceholder = function (str) {
        return rxPlaceholder.test(str)
    }, rxAllWhitespace = /^\s*$/, allWhitespace = function (str) {
        return rxAllWhitespace.test(str)
    }, rta = function (buttonStr, opts) {
        $("textarea.rta")[plugin_name](buttonStr, opts)
    };

    function toggleHTML(btnName, event, button) {
        return rta.toggleHTML(btnName, event, button)
    }

    function getLinkUrl() {
        return rta.getLinkUrl()
    }

    function getImageUrl() {
        return rta.getImageUrl()
    }

    function getFontColor() {
        return rta.getFontColor()
    }

    function getFontBgColor() {
        return rta.getFontBgColor()
    }

    function getHeading() {
        return rta.getHeading()
    }

    function getFontName() {
        return rta.getFontName()
    }

    function getFontSize() {
        return rta.getFontSize()
    }
    ...
    rta.getFontSize = function () {
        var valStr = rta.winPrompt("Font size (1 to 7):"), valInt;
        if (valStr) {
    ...
    rta.toggleHTML = function (btnName, event, button) {
        var iconGroup = $(button).closest(".rta-icon-group")[0], jta = $(iconGroup.rtaTextArea), je = $(iconGroup.rtaEditable);
    ...
    rta.buttonClick = function (btnName, event, button) {
        var btn = rta.button[btnName], val = btn["value"], cmd = btn["cmd"];
        if ($.isFunction(cmd)) {
    ...
    $.fn[plugin_name] = function (param1, param2) {
        return this.each(function () {
            ...
            var rtaGroup = jthis.prev(), iconGroup = $(" > .rta-icon-group", rtaGroup), rtaEditable = $(" > .rta-editable", rtaGroup);
            ...
            iconGroup.click(function (event) {
                var btnName = null, target = event.target, tagName = target.tagName.toLowerCase();
            ...
            if (placeholder) {
                ...
                this.value = this.value.replace(rxPlaceholder, "");
